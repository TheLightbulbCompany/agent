...
`"shared"` to let sessions on the same workspace, agent, and `mcp.servers`
config reuse one already-connected runtime (reaped by the idle sweep, not by
per-session disposal). One-shot embedded runs such as auth probes,
