...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - by default each gateway session gets its own bundled MCP runtime (`mcp.runtimeScope: "session"`); single-tenant deployments can opt into `"shared"` to let sessions on the same workspace, agent, and `mcp.servers` config reuse one already-connected runtime and skip the per-session cold start (the shared runtime is reaped by the idle sweep rather than per-session disposal), at the cost of cross-session isolation between MCP children
