 ...
 * resolves only when `settleWarm()` is called, letting the test prove that
 * materialization does NOT block on the connect.
 */
function makeColdRuntime(
  params: { tools?: McpCatalogTool[]; result?: CallToolResult } = {},
): {
...
describe("materializeBundleMcpToolsForRunNonBlocking", () => {
  ...
  it("round-trips a warm tool's execute through callTool", async () => {
    ...
    await materializeBundleMcpToolsForRunNonBlocking({ runtime });
    settleWarm();
    await new Promise((resolve) => setImmediate(resolve));

    const warm = await materializeBundleMcpToolsForRunNonBlocking({ runtime });
    const result = await warm.tools[0].execute("call-bundle-probe", {}, undefined, undefined);

    expect((result.content[0] as { text?: string }).text).toBe("PONG");
  ...
  it("does not start a background warm when the catalog is already warm", async () => {
    ...
    // Second call sees a warm peekCatalog → no new getCatalog, no new lease.
    const warm = await materializeBundleMcpToolsForRunNonBlocking({ runtime });
    expect(getCatalogCalls()).toBe(1);
    expect(leaseBalance()).toBe(0);
    expect(warm.tools.map((tool) => tool.name)).toEqual(["bundleProbe__bundle_probe"]);
