 ...
 *   (`mcp.sessionIdleTtlMs`), never by per-session disposal. Intended for
 *   single-tenant per-owner containers, where every session is the same owner and
 *   sharing collapses an otherwise per-session MCP connect storm on EFS.
 */
